...
#EE0 = -1j / zR * (1 - ro ** 2 * k0 / 2 / zR) * np.exp(-ro ** 2 * k0 / 2 / zR) #Gauss-Laguerre
EE0 = np.exp(-(ro/rb)**2)
...
for z in Z:
    ...
    zR = k0 * w0 ** 2 / 2
    q = z + 1j * zR
    E = (np.max(EE[1]) * 1j * zR / q * np.exp(-1j * k0 * ro ** 2 / 2 / q)).conj()
    sig = 1 / (2j * (z - 1j * zR))
    #E = -4j * zR * sig ** 2 * (1 - sig * ro ** 2 * k0) * np.exp(-sig * ro ** 2 * k0) #Gauss-Laguerre n1 m0
    EG.append(E)
...
#%%
sd.updateresult(np.array(EE), np.array(EG))
